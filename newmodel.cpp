...
int newModel_cb_byteWidth = 112;	// The size of the combined buffer bytes. Always update after a const buffer struct change

HRESULT NewModel::LoadObjModel(char* filename)
{
...
HRESULT NewModel::CreateConstantBuffer()
{
	HRESULT hr = S_OK;
	...
	constant_buffer_desc.ByteWidth = newModel_cb_byteWidth;	// MUST be a multiple of 16, calculate from CB struct
...
HRESULT NewModel::CreateSampler()
{
	HRESULT hr = S_OK;
...
NewModel::NewModel(ID3D11Device* device, ID3D11DeviceContext* context, char* filenameModel)
{
	m_pD3DDevice = device;
...
NewModel::NewModel(ID3D11Device* device, ID3D11DeviceContext* context, char* filenameModel, char* filenameTexture)
{
	m_pD3DDevice = device;
	m_pImmediateContext = context;
...
XMVECTOR NewModel::GetBoundingSphereWorldSpacePosition(XMMATRIX* objectWorld)
{
	...
	return XMVector3Transform(offset, *objectWorld);
}
